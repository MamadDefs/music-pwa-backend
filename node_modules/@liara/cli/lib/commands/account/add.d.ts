import Command from '../../base';
export default class AccountAdd extends Command {
    static description: string;
    static flags: {
        email: import("@oclif/core/lib/interfaces").OptionFlag<string | undefined>;
        password: import("@oclif/core/lib/interfaces").OptionFlag<string | undefined>;
        account: import("@oclif/core/lib/interfaces").OptionFlag<string | undefined>;
        'from-login': import("@oclif/core/lib/interfaces").BooleanFlag<boolean>;
        help: import("@oclif/core/lib/interfaces").BooleanFlag<void>;
        dev: import("@oclif/core/lib/interfaces").BooleanFlag<boolean>;
        debug: import("@oclif/core/lib/interfaces").BooleanFlag<boolean>;
        'api-token': import("@oclif/core/lib/interfaces").OptionFlag<string | undefined>;
        region: import("@oclif/core/lib/interfaces").OptionFlag<string | undefined>;
    };
    run(): Promise<void>;
    promptName(email: string, region: string): Promise<string>;
    promptEmail(): Promise<string>;
    promptPassword(): Promise<string>;
}
